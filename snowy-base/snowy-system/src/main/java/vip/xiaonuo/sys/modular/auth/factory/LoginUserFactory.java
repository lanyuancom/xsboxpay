                ...
                apps.forEach(dict -> {
                    ...
                    if (ObjectUtil.isNotNull(tempList)) {
                        ...
                        if(sysLoginUser.getAdminType() == 1){
                            loginMenuTreeNodes.stream().forEach(v -> {
                                List ch = v.getChildren();
                                ch = (List) ch.stream().filter(p ->  (!"/payOrder/payindex".equals(((LoginMenuTreeNode)p).getValue()) && !"/mchInfo/info".equals(((LoginMenuTreeNode)p).getValue()))).collect(Collectors.toList());
                                v.setChildren(ch);
                            });
                        }
