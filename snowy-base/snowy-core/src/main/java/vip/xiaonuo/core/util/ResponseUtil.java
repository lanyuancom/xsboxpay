...
public class ResponseUtil {
    ...
    public static void handleErrorWithWrite(Integer code,
        ...
        if(ajaxRequest) {
            ...
            if(code.equals(PermissionExceptionEnum.NO_PERMISSION.getCode()) ||
                    code.equals(PermissionExceptionEnum.NO_PERMISSION_OPERATE.getCode())) {
                //跳转到全局错误处理，没有权限访问页面，403页面
                response.sendRedirect(request.getContextPath() + "/global/sessionError?tags="+tags+"&errorCode=" +  PermissionExceptionEnum.NO_PERMISSION.getCode());
            } else if(code.equals(PermissionExceptionEnum.URL_NOT_EXIST.getCode())) {
                //跳转到全局错误处理，路径不存在，404页面
                response.sendRedirect(request.getContextPath() + "/global/sessionError?tags="+tags+"&errorCode=" +  PermissionExceptionEnum.URL_NOT_EXIST.getCode());
            } else if(code.equals(AuthExceptionEnum.REQUEST_TOKEN_EMPTY.getCode())) {
                //跳转到全局错误处理，登录页，请先登录
                response.sendRedirect(request.getContextPath() + "/global/sessionError?tags="+tags+"&errorCode=" +  AuthExceptionEnum.NOT_LOGIN.getCode());
            } else {
                //跳转到全局错误处理，登录过期需要登录
                response.sendRedirect(request.getContextPath() + "/global/sessionError?tags="+tags);
            }
        }
    }
    ...
    public static boolean isAjaxRequest(HttpServletRequest request) {
        return null != request || !ServletUtil.isGetMethod(request) ||  null != request.getHeader(CommonConstant.ACCEPT)  ||
